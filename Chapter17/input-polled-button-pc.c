...
static void polled_btn_poll(struct input_polled_dev *poll_dev)
{
...
#if 0
	input_report_key(poll_dev->input, KEY_1, 0);
	...
	if (!flag) {
		...
		input_report_key(poll_dev->input, KEY_1, 1);
...
static int polled_btn_probe(struct platform_device *pdev)
{
    ...
    poll_dev->poll_interval = 500; /* Poll every 200ms */
    ...
    set_bit(KEY_1, input_dev->keybit); /* buttons */
